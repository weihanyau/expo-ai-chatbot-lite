...
import { callChatApi} from "../../ui-utils/src/call-chat-api";
...
const processResponseStream = async (
	...
	const response = callChatApi({
		...
		restoreMessagesOnFailure() {
			console.log('Request failed, restoring previous messages');
		...
		onUpdate(merged, data) {
			...
			if (data?.length) {
				mutateStreamData([...(existingDataRef.current ?? []), ...data], false);
			}
		},
		onToolCall,
		onFinish: (...args) => {
...
export function useChat({
	...
	experimental_throttle: throttleWaitMs = 20,
}: UseChatOptions & {
	key?: string;
